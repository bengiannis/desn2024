...
function draw() {
  ...
  for (let yReal = 0; yReal < rows; yReal++) {
    ...
    for (let xReal = 0; xReal < cols; xReal++) {
      ...
      let noiseValue = perlinNoise([distortedX, distortedY, noiseZ], 100/(width + height), 1, 0.63);
  ...
  for (let i = 0; i < 40; i++) {
    ...
    const copyWidth = (Math.floor(perlinNoise([noiseZ/3+(i+10)*200], 0.2, 1, 1) * snap) / snap) * 100 + 1;
    const copyHeight = (Math.floor(perlinNoise([noiseZ/3+(i+30)*200], 0.2, 1, 1) * snap) / snap) * 100 + 1;
    const copyX = (Math.floor(perlinNoise([noiseZ/3+(i)*200], 0.2, 1, 1) * snap) / snap) * window.innerWidth;
    const copyY = (Math.floor(perlinNoise([noiseZ/3+(i+1)*200], 0.2, 1, 1) * snap) / snap) * window.innerHeight;
    const pasteX = (Math.floor(perlinNoise([noiseZ/3+(i+2)*200], 0.2, 1, 1) * snap) / snap) * window.innerWidth;
    const pasteY = (Math.floor(perlinNoise([noiseZ/3+(i+3)*200], 0.2, 1, 1) * snap) / snap) * window.innerHeight;

    let capturedImage = get(copyX, copyY, copyWidth, copyHeight);
    image(capturedImage, pasteX, pasteY);
  }
